...
public class PlayerMovement : MonoBehaviour
{
    ...
    public bool IsPlayerTwo { get => isPlayerTwo; set => isPlayerTwo = value; }
    ...
    void OnInputEvent(string action, BUTTONSTATE buttonState)
    {
        ...
        {
            if (action == "JumpP2" && buttonState == BUTTONSTATE.PRESS && IsGrounded() && playerState.currentState != UNITSTATE.JUMPING) JumpNextFixedUpdate = true;

            //start running when a run button is pressed (e.g. Joypad controls)
            if (action == "RunP2") playerState.SetState(UNITSTATE.RUN);
            //start a jump
        }
