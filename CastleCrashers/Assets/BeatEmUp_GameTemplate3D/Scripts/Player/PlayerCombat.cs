...
public class PlayerCombat : MonoBehaviour, IDamagable<DamageObject>
{
    ...
    public bool IsPlayerTwo { get => _isPlayerTwo; set => _isPlayerTwo = value; }
    ...
    void OnDisable()
    {
        ...
        {
            InputManager.onDirectionInputEventP2 -= OnDirectionInputEvent;
        }
    }
    ...
    private void OnInputEvent(string action, BUTTONSTATE buttonState)
    {
        if (AttackStates.Contains(playerState.currentState) && !isDead)
        {
            ...
            {
                //running punch
                if (action == "PunchP2" && buttonState == BUTTONSTATE.PRESS && playerState.currentState == UNITSTATE.RUN && isGrounded)
                {
                    animator.SetAnimatorBool("Run", false);
                    if (RunningPunch.animTrigger.Length > 0) doAttack(RunningPunch, UNITSTATE.ATTACK, "PunchP2");
                    return;
                }

                //running kick
                if (action == "KickP2" && buttonState == BUTTONSTATE.PRESS && playerState.currentState == UNITSTATE.RUN && isGrounded)
                {
                    animator.SetAnimatorBool("Run", false);
                    if (RunningKick.animTrigger.Length > 0) doAttack(RunningKick, UNITSTATE.ATTACK, "KickP2");
                    return;
                }

                //pick up an item
                if (action == "PunchP2" && buttonState == BUTTONSTATE.PRESS && itemInRange != null && isGrounded && currentWeapon == null)
                {
                    interactWithItem();
                    return;
                }
                ...
                //use an weapon
                if (action == "PunchP2" && buttonState == BUTTONSTATE.PRESS && isGrounded && currentWeapon != null)
                {
                    useCurrentWeapon();
                    return;
                }
                ...
                //ground punch
                if (action == "PunchP2" && buttonState == BUTTONSTATE.PRESS && (playerState.currentState != UNITSTATE.PUNCH && NearbyEnemyDown()) && isGrounded)
                {
                    if (GroundPunchData.animTrigger.Length > 0) doAttack(GroundPunchData, UNITSTATE.GROUNDPUNCH, "PunchP2");
                    return;
                }

                //ground kick
                if (action == "KickP2" && buttonState == BUTTONSTATE.PRESS && (playerState.currentState != UNITSTATE.KICK && NearbyEnemyDown()) && isGrounded)
                {
                    if (GroundKickData.animTrigger.Length > 0) doAttack(GroundKickData, UNITSTATE.GROUNDKICK, "KickP2");
                    return;
                }
                ...
                //default punch
                if (action == "PunchP2" && buttonState == BUTTONSTATE.PRESS && playerState.currentState != UNITSTATE.PUNCH && playerState.currentState != UNITSTATE.KICK && isGrounded)
                {
                    ...
                    if (PunchCombo[attackNum] != null && PunchCombo[attackNum].animTrigger.Length > 0) doAttack(PunchCombo[attackNum], UNITSTATE.PUNCH, "PunchP2");
                    return;
                }

                //advance the punch combo if "punch" was pressed during a punch attack
                if (action == "PunchP2" && buttonState == BUTTONSTATE.PRESS && (playerState.currentState == UNITSTATE.PUNCH) && !continuePunchCombo && isGrounded)
                {
                    if (attackNum < PunchCombo.Length - 1)
                    {
                        continuePunchCombo = true;
                ...
                //jump punch
                if (action == "PunchP2" && buttonState == BUTTONSTATE.PRESS && !isGrounded)
                {
                    if (JumpKickData.animTrigger.Length > 0)
                    {
                        doAttack(JumpKickData, UNITSTATE.JUMPKICK, "KickP2");
                        StartCoroutine(JumpKickInProgress());
                    }
                    return;
                }
                ...
                //jump kick
                if (action == "KickP2" && buttonState == BUTTONSTATE.PRESS && !isGrounded)
                {
                    if (JumpKickData.animTrigger.Length > 0)
                    {
                        doAttack(JumpKickData, UNITSTATE.JUMPKICK, "KickP2");
                ...
                //default kick
                if (action == "KickP2" && buttonState == BUTTONSTATE.PRESS && playerState.currentState != UNITSTATE.KICK && playerState.currentState != UNITSTATE.PUNCH && isGrounded)
                {
                    ...
                    doAttack(KickCombo[attackNum], UNITSTATE.KICK, "KickP2");
                    return;
                }

                //advance the kick combo if "kick" was pressed during a kick attack
                if (action == "KickP2" && buttonState == BUTTONSTATE.PRESS && (playerState.currentState == UNITSTATE.KICK) && !continueKickCombo && isGrounded)
                {
                    if (attackNum < KickCombo.Length - 1)
                    {
                        continueKickCombo = true;
